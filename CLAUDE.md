...
|---|---|---|
| `handoff` | `/handoff` | Session-end handoff (`.ai/HANDOFF.md` + `update-claude-md`) |
| `resume` | `/resume` | Reconstruct session context at start |
| `update-claude-md` | `/update-claude-md` | Promote session knowledge into `CLAUDE.md` |
| `grill-me` | `/grill-me` | Stress-test a plan via structured questioning |
| `lit-review` | `/lit-review` | Scientific literature search/synthesis (Zotero write needs `ZOTERO_*` env vars in `~/.claude/settings.json`) |
| `overbaked` | `/overbaked` | Audit for over-engineering and scope creep |
| `slack-message` | `/slack-message` | Draft internal Slack message from git context |
| `write-new-skill` | `/write-new-skill` | Scaffold new skills |
| `unstale` | `/unstale` | Detect/repair staleness residue; `--auto` applies HIGH-confidence fixes |
| `figure-review` | `/figure-review` | Publication-readiness audit for figures; `--style` adds CC house style |
| `reviewer-2` | `/reviewer-2` | Adversarial per-claim stress-test |
| `pathfinder` | `/pathfinder` | Router: every skill + subagent and when to use each |
| `ai-review` | `/ai-review` | Parallel orchestrated repo review + gap hunting/ideation/synthesis; run on fable at high+ effort |
...
The 5-hour usage window is **rolling and anchored to the first real session message** — and only a genuine **`claude -p`** session anchors it (a claude.ai **cloud routine** spends tokens but does *not* anchor; single test 2026-06-24 — **do not** move it back to a cloud routine, and re-verify after major Claude Code/plan changes). Anchoring at ~5/10/15:00 ET weekdays runs in **three independent tiers**; a minimal Haiku ping inside an already-open window is a harmless no-op, so running all three is safe. Assets in `window-warmup/`; full rationale, empirics, and setup in `window-warmup/README.md`.

- **Tier 1 — macOS launchd** (`bash window-warmup/install.sh`): best-effort local redundancy — fires late when the Mac is asleep (mechanism unconfirmed) and not at all when powered off. Log: `~/.claude/window-warmup.log`.
