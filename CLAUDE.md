...
- **User-invoked** (`disable-model-invocation: true`) — orchestrators you type explicitly: `grill-me`, `handoff`, `resume`, `slack-message`, `create-alert`, `tab-setup`, `write-new-skill`, `pathfinder`, `ai-review`, `commit-batch`, `repo-init`.
...
- **Companion files: use a relative Markdown link, not a shell load.** A plugin installs under `~/.claude/plugins/cache/…`, *not* `~/.claude/skills/`, so a companion file's path differs by install mode. The repo used to bridge that with `${CLAUDE_PLUGIN_ROOT:+$CLAUDE_PLUGIN_ROOT/skills/<name>}` + a `$HOME` fallback in a `` ```! `` preamble (and that *did* work — confirmed 2026-07-20 that Claude Code exports the var into preambles, not just hooks/commands). **That pattern is retired: it is bash parameter expansion, and preambles run under PowerShell on Windows, where it throws and takes the whole skill down.** Use a **relative link** instead — `[TEMPLATES.md](TEMPLATES.md)` resolves from the SKILL.md's own directory and is therefore correct under both install modes with no shell at all. `lit-review`'s `REFERENCE.md` was always the right model; `figure-review` (`COLORBLIND.md`, `CC-STYLE.md`) and `repo-init` (`TEMPLATES.md`) now follow it. **A link is lazy where a preamble was eager**, so pair it with an explicit instruction saying *when* to read it and what to do if it is missing — and where the content is short and needed on every invocation, prefer **inlining it into SKILL.md** over any companion file. `create-alert`'s injection-hardening block took that route: 12 lines, required verbatim in every generated routine prompt, and a silent skip would have been a security regression rather than a cosmetic one, so indirection bought nothing.
