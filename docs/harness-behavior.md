...
| 3 | Prose *before* a command suppresses expansion entirely — `let's /pickup now` and `let's /overbaked now` both arrived as raw user text, no `<command-name>` block, no skill body. | observed | **n=3, gating and body-residency both eliminated.** The first pair was chosen to discriminate on gating: `pickup` is `disable-model-invocation: true`, `overbaked` is not, and both were suppressed identically — so "gated skills don't expand mid-sentence" is ruled out. The third trial (2026-08-04, **CLI 2.1.221**) is the cold-body control the first two lacked: `let's /overbaked now` in a session where `overbaked` had never expanded, so its body was not resident, and suppression was identical. That closes the "the negative is weak because the content was already there" objection. All submissions were unbackticked plain prose (checked; backtick-wrapping is not the mechanism either). **Still open:** all three used the same `let's … now` scaffold, so phrasing is not separated from position; and the cold trial was the session's *second* submission, so a strict first-submission-only variant of position is not excluded — nothing observed suggests one. |
...
- Row 3 as the **first submission of a fresh session** — the cold-*body* control is done (see row 3, n=3), but every trial so far has been a session's second-or-later submission, so "position within the submission" is not yet separated from "position within the session." Submit `let's /pickup now` immediately after a `/clear`, before any command expands.
- Row 3 with a **different prose prefix** — all three trials used `let's … now`. Cheapest folded into the trial above by varying the scaffold.
